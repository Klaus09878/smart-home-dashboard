...
- `app.js` — gesamte Hub-/ClimateFlow-Logik (appState, fetchFeeds, Charts, Lüftungsberater, Schimmelrisiko, Prognose, Archiv-Ansicht, Hub-Widgets, ntfy-Client)
...
- `functions/api/` — `feeds/[locId].js` (ThingSpeak-Proxy, Env `TS_KEY_GILLIAN`/`TS_KEY_SEAN`), `gpx.js` (D1-CRUD mit Tombstones/`updated_at`), `climate.js` (Tages-Aggregate), `check-alerts.js` (ntfy, Env `NTFY_TOPIC`); D1-Binding heißt `DB`, Schema wird zur Laufzeit angelegt
