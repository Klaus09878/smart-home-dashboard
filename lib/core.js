...
(function (root, factory) {
  ...
  // Klima-Budget (Plan7-11): wie viele Tage lag das Raumklima im Zielkorridor
  // (Temp UND Feuchte innerhalb der Schwellen)? Liefert Quote und die aktuelle
  // Serie (aufeinanderfolgende gruene Tage vom letzten bekannten Tag rueckwaerts).
  // rows = [{ day:'YYYY-MM-DD', t_avg, h_avg }, …]; th = Schwellen.
  function comfortBudget(rows, th = {}, opts = {}) {
    const tMin = th.tempMin ?? 19, tMax = th.tempMax ?? 24;
    const hMin = th.humMin ?? 40, hMax = th.humMax ?? 60;
    if (!Array.isArray(rows)) return null;
    const clean = rows
      .filter(r => r && r.day && r.t_avg != null && r.h_avg != null && !isNaN(r.t_avg) && !isNaN(r.h_avg))
      .slice()
      .sort((a, b) => (a.day < b.day ? -1 : a.day > b.day ? 1 : 0));
    if (!clean.length) return null;
    const isGreen = r => r.t_avg >= tMin && r.t_avg <= tMax && r.h_avg >= hMin && r.h_avg <= hMax;
    const greenDays = clean.filter(isGreen).length;
    const total = clean.length;
    let streak = 0;
    for (let i = clean.length - 1; i >= 0; i--) { if (isGreen(clean[i])) streak++; else break; }
    return { totalDays: total, greenDays, pct: Math.round((greenDays / total) * 100), streak };
  }
