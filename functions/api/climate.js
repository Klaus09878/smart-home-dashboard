...
async function ensureSchema(db) {
  ...
  // Tagesnotiz (P3-7)
  try { await db.exec("ALTER TABLE climate_daily ADD COLUMN note TEXT"); } catch (e) { /* existiert */ }
}
...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
    ...
    // ON CONFLICT statt INSERT OR REPLACE: erhaelt eine evtl. vorhandene Tagesnotiz
    const stmt = env.DB.prepare(
      'INSERT INTO climate_daily (loc, day, t_min, t_max, t_avg, h_min, h_max, h_avg, samples, co2_avg, co2_max) VALUES (?,?,?,?,?,?,?,?,?,?,?) ' +
      'ON CONFLICT(loc, day) DO UPDATE SET t_min=excluded.t_min, t_max=excluded.t_max, t_avg=excluded.t_avg, h_min=excluded.h_min, h_max=excluded.h_max, h_avg=excluded.h_avg, samples=excluded.samples, co2_avg=excluded.co2_avg, co2_max=excluded.co2_max'
    );
  ...
  // Tagesnotiz setzen/loeschen (P3-7)
  if (request.method === 'PUT') {
    const b = await request.json();
    if (!b.loc || !b.day) return json({ error: 'loc und day erforderlich' }, 400);
    const note = (b.note == null || String(b.note).trim() === '') ? null : String(b.note).slice(0, 280);
    await env.DB.prepare(
      'INSERT INTO climate_daily (loc, day, note) VALUES (?,?,?) ON CONFLICT(loc, day) DO UPDATE SET note=excluded.note'
    ).bind(b.loc, b.day, note).run();
    return json({ ok: true });
  }
