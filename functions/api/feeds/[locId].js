...
export async function onRequestGet(context) {
  ...
  if (!loc) return json({ error: 'Unbekannter Standort' }, 404);
  ...
  let upstream = `https://api.thingspeak.com/channels/${loc.channel}/feeds.json?api_key=${apiKey}&results=${encodeURIComponent(results)}`;
