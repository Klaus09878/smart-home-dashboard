...
//
// Werte werden als ROHER String gespeichert (Store liefert immer Strings; JSON
// steckt ggf. schon drin). So kann sowohl der Client (Store.getJSON) als auch
// der Server (_notify.js) sie eindeutig einmal parsen. value === null löscht.
// Das Profil wird IMMER aus dem Login abgeleitet (identify), nie aus dem Body.
// Konfliktlösung: neuerer updatedAt gewinnt (Last-Write-Wins).
...
export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    ...
    for (const row of results) {
      settings[row.key] = { value: row.value === null ? null : row.value, updatedAt: row.updated_at };
    }
  ...
  if (request.method === 'POST') {
    ...
    for (const item of items) {
      ...
      // value ist immer String oder null (Löschung); nie erneut JSON-kodieren
      const value = item.value === null || item.value === undefined ? null : String(item.value);
      if (value !== null && value.length > 100_000) continue;
