...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
    ...
    const stmt = env.DB.prepare('INSERT OR REPLACE INTO climate_daily (loc, day, t_min, t_max, t_avg, h_min, h_max, h_avg, samples) VALUES (?,?,?,?,?,?,?,?,?)');
    const batch = body.days.map(d =>
      stmt.bind(body.loc, d.day, d.tMin ?? null, d.tMax ?? null, d.tAvg ?? null, d.hMin ?? null, d.hMax ?? null, d.hAvg ?? null, d.samples ?? null)
    );
